// reducers/counter.js
import { leftSideMenus } from '../constant/menus'
// reducer가 많아지면 action상수가 중복될 수 있으니
// 액션이름 앞에 파일 이름을 넣습니다.
export const CHOICE = "CHOICE/MENU";

export const choiceMenus = (index) => ({ type: CHOICE, index })


const initState = {
    menus : leftSideMenus
}
export const menus = (state = initState, action) => {
  
  switch (action.type) {
    case CHOICE :
        const copyMenus = [ ...state.menus ].map( menu => ({...menu, selectedTF : (action.index === menu.index )}))
        
      return {
        ...state,
        menus : copyMenus
      };

    // default를 쓰지 않으면 맨처음 state에 count값이 undefined가 나옵니다 꼭! default문을 넣으세요
    default:
      return state;
  }
};